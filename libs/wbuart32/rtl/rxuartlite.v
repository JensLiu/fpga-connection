...
`endif
    ...
    always @(posedge i_clk)
        if (i_reset) {ck_uart, qq_uart, q_uart} <= 3'b111;
        else {ck_uart, qq_uart, q_uart} <= {qq_uart, q_uart, i_uart_rx};
    // }}}
    ...
    always @(posedge i_clk)
        if (i_reset) chg_counter <= {(TB) {1'b1}};
        else if (qq_uart != ck_uart) chg_counter <= 0;
    ...
    always @(posedge i_clk)
        if (i_reset) half_baud_time <= 0;
        else half_baud_time <= (!ck_uart) && (chg_counter >= half_baud - 1'b1 - 1'b1);
    // }}}
    ...
    always @(posedge i_clk)
        if (i_reset) begin
            state <= RXUL_IDLE;
        end else if (state == RXUL_IDLE) begin  // Idle state, independent of baud counter
            // {{{
    ...
    always @(posedge i_clk)
        if (i_reset) begin
            o_wr   <= 1'b0;
            o_data <= 8'h00;
        end else if ((zero_baud_counter) && (state == RXUL_STOP) && (ck_uart)) begin
            o_wr   <= 1'b1;
            o_data <= data_reg;
        end else o_wr <= 1'b0;
    // }}}
    ...
    always @(posedge i_clk)
        if (i_reset) zero_baud_counter <= 1'b1;
        else if ((state == RXUL_IDLE) && (!ck_uart) && (half_baud_time)) zero_baud_counter <= 1'b0;
...
`ifdef FORMAL
    ...
    always @(posedge gbl_clk) f_rx_clock <= f_rx_clock + 1'b1;

    always @(*) assume (i_clk == f_rx_clock[1]);

    always @(posedge gbl_clk) if (!$rose(i_clk)) assume (!$fell(i_reset));
    ...
    always @(posedge gbl_clk) f_tx_clock <= f_tx_clock + f_tx_step;
    ...
    always @(*) if (i_reset) assume (i_uart_rx);

    ////////////////////////////////////////////////////////////////////////
    //
    // The simulated timing generator

    always @(*) if (i_reset) assume (!f_tx_busy);

    always @(*) if (f_tx_busy || i_reset) assume (!f_tx_start);

    always @(*) if (i_reset) assume (f_tx_baud == CLOCKS_PER_BAUD - 1);

    initial f_tx_baud = 0;
    always @(posedge f_txclk)
        if (f_tx_zbaud && (f_tx_busy || f_tx_start)) f_tx_baud <= CLOCKS_PER_BAUD - 1;
        else if (!f_tx_zbaud) f_tx_baud <= f_tx_baud - 1;
    ...
    always @(posedge f_txclk)
        ...
        end else if (f_tx_reg[9]) begin
            assert (f_tx_count == CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else if (f_tx_reg[8]) begin
            assert (f_tx_count == 2 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else if (f_tx_reg[7]) begin
            assert (f_tx_count == 3 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else if (f_tx_reg[6]) begin
            assert (f_tx_count == 4 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else if (f_tx_reg[5]) begin
            assert (f_tx_count == 5 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else if (f_tx_reg[4]) begin
            assert (f_tx_count == 6 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else if (f_tx_reg[3]) begin
            assert (f_tx_count == 7 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else if (f_tx_reg[2]) begin
            assert (f_tx_count == 8 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else if (f_tx_reg[1]) begin
            assert (f_tx_count == 9 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else if (f_tx_reg[0]) begin
            assert (f_tx_count == 10 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end else begin
            assert (f_tx_count == 11 * CLOCKS_PER_BAUD - 1 - f_tx_baud);
        end
    ...
    always @(posedge i_clk)
        if (i_reset) f_rx_count <= 0;
        else if (state == RXUL_IDLE) f_rx_count <= (!ck_uart) ? (chg_counter + 2) : 0;
        else f_rx_count <= f_rx_count + 1'b1;

    always @(posedge i_clk)
        case (state)
            0: assert (f_rx_count == half_baud + (CLOCKS_PER_BAUD - baud_counter));
            1: assert (f_rx_count == half_baud + 2 * CLOCKS_PER_BAUD - baud_counter);
            2: assert (f_rx_count == half_baud + 3 * CLOCKS_PER_BAUD - baud_counter);
            3: assert (f_rx_count == half_baud + 4 * CLOCKS_PER_BAUD - baud_counter);
            4: assert (f_rx_count == half_baud + 5 * CLOCKS_PER_BAUD - baud_counter);
            5: assert (f_rx_count == half_baud + 6 * CLOCKS_PER_BAUD - baud_counter);
            6: assert (f_rx_count == half_baud + 7 * CLOCKS_PER_BAUD - baud_counter);
            7: assert (f_rx_count == half_baud + 8 * CLOCKS_PER_BAUD - baud_counter);
            8:
   ...
   ||((!zero_baud_counter)&&(baud_counter != 0)));
    ...
    always @(posedge i_clk)
        if (f_past_valid && !$past(
                ...
                state
            ) == RXUL_IDLE && (state == RXUL_IDLE)) begin
   ...
   &&(zero_baud_counter)&&(!f_tx_busy));
    ...
    always @(posedge gbl_clk)
        if (!f_past_valid || i_reset) {ck_tx_clock, q_tx_clock} <= 0;
        else {ck_tx_clock, q_tx_clock} <= {q_tx_clock, f_tx_clock};
    always @(posedge gbl_clk)
        if (!f_past_valid || i_reset) {ck_tx_count, q_tx_count} <= 0;
        else {ck_tx_count, q_tx_count} <= {q_tx_count, f_tx_count};
    ...
    always @(posedge gbl_clk) if (state == RXUL_WAIT) assert ((!f_tx_busy) || (f_tx_reg[9:1] == 0));
    ...
    always @(posedge gbl_clk)
        if (f_past_valid && !$past(i_reset)) begin
            if (state == RXUL_IDLE) begin
                ...
                assert (f_sub_baud_difference <= 3 * ((CLOCKS_PER_BAUD << F_CKRES) / 20));
            end else if (state == 2) begin
                assert (f_sub_baud_difference <= 4 * ((CLOCKS_PER_BAUD << F_CKRES) / 20));
            end else if (state == 3) begin
                assert (f_sub_baud_difference <= 5 * ((CLOCKS_PER_BAUD << F_CKRES) / 20));
            end else if (state == 4) begin
                assert (f_sub_baud_difference <= 6 * ((CLOCKS_PER_BAUD << F_CKRES) / 20));
            end else if (state == 5) begin
                assert (f_sub_baud_difference <= 7 * ((CLOCKS_PER_BAUD << F_CKRES) / 20));
            end else if (state == 6) begin
                assert (f_sub_baud_difference <= 8 * ((CLOCKS_PER_BAUD << F_CKRES) / 20));
            end else if (state == 7) begin
                assert (f_sub_baud_difference <= 9 * ((CLOCKS_PER_BAUD << F_CKRES) / 20));
            end else if (state == 8) begin
                assert (f_sub_baud_difference <= 10 * ((CLOCKS_PER_BAUD << F_CKRES) / 20));
            end
        end
    ...
    ////////////////////////////////////////////////////////////////////////
    //
    always @(posedge i_clk) cover (o_wr);  // Step 626, takes about 20mins

    always @(posedge i_clk)
        if (!i_reset && f_past_valid && !$past(i_reset)) begin
            cover (!ck_uart);
