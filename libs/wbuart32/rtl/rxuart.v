...
module rxuart #(
    ...
    always @(posedge i_clk)
        if (i_reset) {ck_uart, qq_uart, q_uart} <= 3'h0;
    ...
    always @(posedge i_clk)
        if (i_reset) o_break <= 1'b0;
        else o_break <= ((chg_counter >= break_condition) && (~ck_uart)) ? 1'b1 : 1'b0;
    // }}}
    ...
    always @(posedge i_clk)
        if (i_reset) line_synch <= 1'b0;
        else line_synch <= ((chg_counter >= break_condition) && (ck_uart));
    // }}}
    ...
    always @(posedge i_clk)
        if (i_reset) half_baud_time <= 1'b0;
        else half_baud_time <= (~ck_uart) && (chg_counter >= {4'h0, half_baud});
    // }}}
    ...
    always @(posedge i_clk)
        if (i_reset) r_setup <= INITIAL_SETUP[29:0];
        else if (state >= RXU_RESET_IDLE) r_setup <= i_setup[29:0];
    // }}}
 ...
 begin // Goto idle state following return ck_uart going high
    ...
    always @(posedge i_clk)
        if (i_reset) calc_parity <= 0;
        else if (state == RXU_IDLE) calc_parity <= 0;
    ...
    initial o_wr = 1'b0;
    always @(posedge i_clk)
        if (i_reset) o_wr <= 1'b0;
        else if ((zero_baud_counter) || (state == RXU_IDLE)) o_wr <= (pre_wr) && (!i_reset);
        else o_wr <= 1'b0;
    // }}}
    ...
    // intervals.
    always @(posedge i_clk)
        if (i_reset) baud_counter <= INITIAL_SETUP[23:0] - 1;
        else if (zero_baud_counter) baud_counter <= clocks_per_baud - 1;
        else
            case (state)
    ...
    always @(posedge i_clk)
        ...
        else zero_baud_counter <= (baud_counter == 1);
    // }}}
endmodule
