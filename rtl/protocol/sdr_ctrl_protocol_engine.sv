...
module sdr_ctrl_protocol_engine (
    ...
    sdr_ctrl_protocol_if.master req_stream,
    sdr_ctrl_protocol_if.slave rsp_stream
);
  ...
  logic sdr_is_ctl = rsp_stream.valid && rsp_stream.data.is_control;
  logic sdr_is_data = rsp_stream.valid && ~rsp_stream.data.is_control;
  logic sdr_ctl_ready = sdr_is_ctl && rsp_stream.data.payload.control.sdr_ready;
  logic sdr_ctl_stop = sdr_is_ctl && ~rsp_stream.data.payload.control.sdr_ready;
  logic sdr_ctl_tx_ack = sdr_is_ctl && rsp_stream.data.is_ack && rsp_stream.data.sdr_tx;
  logic sdr_ctl_rx_ack = sdr_is_ctl && rsp_stream.data.is_ack && ~rsp_stream.data.sdr_tx;
  logic sdr_data_ack = sdr_is_data && rsp_stream.data.is_ack;
  // always_comb begin
  //   $assert(~req_stream.valid || req_stream.data.sop && req_stream.data.eop);
  //   $assert(~rsp_stream.valid || rsp_stream.data.sop && rsp_stream.data.eop);
  // end
    ...
    case (state)
      ...
      S_IDLE: begin
        if (core_is_read) begin
          ...
          req_stream.valid = 1;
          req_stream.data.is_control = 1;
          req_stream.data.payload.control.sdr_tx = 1;
          if (sdr_ctl_tx_ack) begin
            // $assert(req_stream.ready);  // < Blocking interface, send before receiving
            state_n = S_TX_IDLE;
          end
          ...
          req_stream.valid = 1;
          req_stream.data.is_control = 1;
          req_stream.data.payload.control.sdr_rx = 1;
          if (sdr_ctl_rx_ack) begin
            // $assert(req_stream.ready);
            state_n = S_RX_IDLE;
          end
        end
      end
      ...
      S_RX_STREAMING: begin
        ...
        core.rd_data_size = rsp_stream.data.payload.data.payload_size;
        core.rd_data = rsp_stream.data.payload.data.payload_data;
        if (~core_is_read) begin
      ...
      S_TX_STREAMING: begin
        req_stream.valid = 1;
        req_stream.data.is_control = 0;
        req_stream.data.data.payload_size = core.wr_data_size;
        req_stream.data.data.payload = core.wr_data;
